    ...
    <aside className="flex flex-col h-screen flex-shrink-0" style={{ width: '224px', background: 'var(--surface)', borderRight: '1px solid var(--border)' }}>
        ...
        {navItems.map(({ to, icon: Icon, label, exact }) => {
          ...
          return (
            <Link key={to} to={to}
              ...
              style={{ background: active ? 'var(--primary-muted)' : 'transparent', color: active ? 'var(--primary)' : 'var(--text-secondary)', border: active ? '1px solid var(--primary-border)' : '1px solid transparent', textDecoration: 'none' }}>
              <Icon size={17} />{label}
            </Link>
          );
        })}
      ...
      {user && (
        <div className="px-3 pb-5 space-y-2" style={{ borderTop: '1px solid var(--border)', paddingTop: '1rem' }}>
          ...
          <button onClick={onLogout} className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium w-full hover:bg-white/5 transition-colors"
